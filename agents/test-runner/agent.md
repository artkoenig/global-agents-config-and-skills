---
name: test-runner
description: "Axis C of the five-axis review: does it still work? Finds the project's test command, runs the suite, and reports green/red with the failing output. Use it as part of the `review` skill, or whenever you just need the suite run without its output landing in the main conversation. It runs and reports only — it never edits code or tests to make them pass."
